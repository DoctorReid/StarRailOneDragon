...
class DivUniChooseBless(SrOperation):

    def __init__(self, ctx: SrContext, skip_initial_screen_check: bool = False):
        """
        选择祝福 点击一次确认后退出
        """
        SrOperation.__init__(self, ctx, op_name=gt('差分宇宙-选择祝福', 'ui'))
    ...
    @operation_node(name='处理选择祝福')
    def handle_choose(self) -> OperationRoundResult:
        screen = self.screenshot()

        pos_list = self.ctx.div_uni_context.get_bless_pos(screen)
        # 暂时没有看到刷新按钮
        ...
        if len(to_choose_list) > 0:
            ...
            return self.round_retry(status='按优先级未匹配到祝福', wait=1)
