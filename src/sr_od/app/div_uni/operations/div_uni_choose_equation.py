...
class DivUniChooseEquation(SrOperation):

    def __init__(self, ctx: SrContext, skip_initial_screen_check: bool = False):
        """
        选择方程 点击一次确认后退出
        """
        SrOperation.__init__(self, ctx, op_name=gt('差分宇宙-选择方程', 'ui'))
    ...
    def handle_choose(self) -> OperationRoundResult:
        ...
        if len(to_choose_list) > 0:
            ...
            return self.round_by_ocr_and_click(screen, '确认', lcs_percent=1,
