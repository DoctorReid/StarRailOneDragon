...
class SimUniReward(StateOperation):
    ...
    NUM_RECT: ClassVar[Rect] = Rect(1060, 715, 1090, 744)  # 沉浸器数字部分
    ...
    def __init__(self, ctx: Context, max_to_get: int,
        ...
        esc = StateOperationNode('退出', self._get_reward)
        ...
        self.on_reward: Optional[Callable[[], None]] = on_reward  # 获取奖励时的回调
    ...
    def _get_reward(self) -> OperationOneRoundResult:
        ...
        pos = self._get_reward_pos(screen)
        if pos is not None:
    ...
    def _get_reward_pos(self, screen: MatLike) -> Optional[Point]:
        """
        获取可以点击的获取奖励按钮
        :param screen:
        :return:
        """
        for rect in [SimUniReward.NUM_RECT, SimUniReward.POWER_NUM_RECT]:  # 优先使用沉浸器
