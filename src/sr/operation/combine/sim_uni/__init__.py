...
class SimUniRoute:
    ...
    def __init__(self, uni_num: int, level: int,
    ...
    def _create_new_route(self):
        ...
        :return:
        """
        base_dir = SimUniRoute.get_uni_base_dir(self.uni_num, self.level)
        self.idx = 1  # 获取合法的下标
        while True:
            route_dir = os.path.join(base_dir, '%02d' % self.idx)
    ...
    def uid(self) -> str:
        ...
        :return:
        """
        return '%02d_%02d_%02d' % (self.uni_num, self.level, self.idx)
    ...
    @staticmethod
    def get_uni_base_dir(uni_num: int, level: int) -> str:
        """
        获取选定宇宙对应的文件夹位置
        :param uni_num:
        :param level:
