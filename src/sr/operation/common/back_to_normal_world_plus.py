...
class BackToNormalWorldPlus(Operation):
    ...
    def _execute_one_round(self) -> OperationOneRoundResult:
        ...
        # 对话框 - 逐光捡金 退出确认
        dialog_confirm = ScreenDialog.FAST_RECOVER_CONFIRM.value
        ...
        if self.find_area(express_supply, screen):
            empty_to_continue = ScreenNormalWorld.EMPTY_TO_CONTINUE.value
            self.ctx.controller.click(empty_to_continue.center)
            time.sleep(3)  # 暂停一段时间再操作
            self.ctx.controller.click(empty_to_continue.center)  # 领取需要分两个阶段 点击两次
