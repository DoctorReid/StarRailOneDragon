...
def _test_search_best_mission_team():
    m01 = TreasuresLightwardTeamModule(module_name='银狼符玄插件', combat_type='', module_type='',
                                       character_id_list=[SILVERWOLF.id, FUXUAN.id])
    m03 = TreasuresLightwardTeamModule(module_name='镜流', combat_type='', module_type='',
                                       character_id_list=[JINGLIU.id, PELA.id])
    m04 = TreasuresLightwardTeamModule(module_name='龙丹', combat_type='', module_type='',
                                       character_id_list=[DANHENGIMBIBITORLUNAE.id, LUOCHA.id])
    m05 = TreasuresLightwardTeamModule(module_name='龙丹4人', combat_type='', module_type='',
                                       character_id_list=[DANHENGIMBIBITORLUNAE.id, HANYA.id, YUKONG.id, LUOCHA.id])
    m06 = TreasuresLightwardTeamModule(module_name='克拉拉', combat_type='', module_type='',
                                       character_id_list=[CLARA.id, TINGYUN.id, YUKONG.id, LUOCHA.id])

    m91 = TreasuresLightwardTeamModule(module_name='符玄', combat_type='', module_type='',
                                       character_id_list=[FUXUAN.id])
    m92 = TreasuresLightwardTeamModule(module_name='罗刹', combat_type='', module_type='',
                                       character_id_list=[LUOCHA.id])
    m93 = TreasuresLightwardTeamModule(module_name='银狼', combat_type='', module_type='',
                                       character_id_list=[SILVERWOLF.id])
    m94 = TreasuresLightwardTeamModule(module_name='佩拉', combat_type='', module_type='',
                                       character_id_list=[PELA.id])
    m95 = TreasuresLightwardTeamModule(module_name='驭空', combat_type='', module_type='',
                                       character_id_list=[YUKONG.id])
    m96 = TreasuresLightwardTeamModule(module_name='停云', combat_type='', module_type='',
                                       character_id_list=[TINGYUN.id])
    m97 = TreasuresLightwardTeamModule(module_name='艾丝妲', combat_type='', module_type='',
                                       character_id_list=[ASTA.id])
    m98 = TreasuresLightwardTeamModule(module_name='寒鸦', combat_type='', module_type='',
    ...
    for type1 in CHARACTER_COMBAT_TYPE_LIST:
        for type2 in CHARACTER_COMBAT_TYPE_LIST:
            ...
            node_team_list = TreasuresLightwardApp.search_best_mission_team(node_combat_types, module_list)
            cn_list = []
