...
class MapCalculator:
    ...
    def auto_cut_map(self, map_image: cv2.typing.MatLike,
        ...
        arrow_mask, angle = self.find_map_arrow_mask(map_image, angle=angle) if is_little_map else (np.zeros(map_image.shape[:2], dtype=np.uint8), angle)
    ...
    def merge_all_map_mask(self, map_image: cv2.typing.MatLike,
        ...
        usage[np.where(arrow_mask == 255)] = [0, 0, 0, 0]
        usage[np.where(road_mask == 255)] = constants.COLOR_MAP_ROAD_BGRA
    ...
    def cal_character_pos_by_feature(self, little_map: cv2.typing.MatLike, large_map: cv2.typing.MatLike, show: bool = False):
        ...
        large_map_bw, large_map_usage, _ = self.auto_cut_map(large_map)
        source = cv2.cvtColor(large_map_usage, cv2.COLOR_BGRA2BGR)
        template = cv2.cvtColor(little_map_usage, cv2.COLOR_BGRA2BGR)
